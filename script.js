...
function createTrackRow(track) {
    ...
    soloBtn.addEventListener('click', () => {
        ...
        soloBtn.style.backgroundColor = track.solo ? '#17a2b8' : '';

        if (track.solo) {
            tracks.forEach(t => {
                if (t !== track) {
                    t.solo = false;
                    const soloButton = t.elem.querySelector('.solo-btn');
                    if (soloButton) {
                        soloButton.style.backgroundColor = '';
                    }
                }
            });
        }
...
function dragEnd() {
    ...
    console.log('headersTbody:', headersTbody);
    console.log('timelineTbody:', timelineTbody);
    ...
    if (timelineTbody) {
        ...
        console.error('Could not find timeline table body.');
    }
...
mainContent.addEventListener('mousedown', (e) => {
    ...
    startX = e.clientX - mainContentRect.left + mainContent.scrollLeft;

    startY = e.clientY - mainContentRect.top + mainContent.scrollTop;
    ...
    document.addEventListener('mousemove', onMouseMoveMarquee);

    document.addEventListener('mouseup', onMouseUpMarquee);
});
...
function onMouseMoveMarquee(e) {
    ...
    let currentX = e.clientX - mainContentRect.left + mainContent.scrollLeft;

    let currentY = e.clientY - mainContentRect.top + mainContent.scrollTop;
    ...
    marqueeBox.style.left = `${width > 0 ? startX : currentX}px`;

    marqueeBox.style.top = `${height > 0 ? startY : currentY}px`;
}
...
function onMouseUpMarquee(e) {
    ...
    document.removeEventListener('mousemove', onMouseMoveMarquee);

    document.removeEventListener('mouseup', onMouseUpMarquee);
}
